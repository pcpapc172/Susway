...
public class UICamera : MonoBehaviour
{
	...
	private void ProcessTouch(bool pressed, bool unpressed)
	{
		...
		float num = ((!flag) ? (DeviceInfo.dpi * 0.08f) : mouseDragThreshold);
		num = DeviceInfo.dpi * 0.08f;
		float num2 = ((!flag) ? (DeviceInfo.dpi * 0.08f) : mouseClickThreshold);
		num2 = DeviceInfo.dpi * 0.08f;
		if (pressed)
		{
			if (mTooltip != null)
			{
